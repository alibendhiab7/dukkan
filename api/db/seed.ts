...
export async function seed() {
  ...
  await turso.execute({ sql: 'INSERT INTO tenant_settings (tenant_id, enable_inventory, enable_sales, enable_reports, enable_employees) VALUES (?, ?, ?, ?, ?)', args: ['300', 1, 1, 1, 0] });

  const sysHash = hashSimple('sysadmin123');
  const adminHash = hashSimple('admin123');
  const salimHash = hashSimple('salim123');
  const omarHash = hashSimple('omar123');
  const aliHash = hashSimple('ali123');
  const nabilHash = hashSimple('nabil123');
  const fatimaHash = hashSimple('fatima123');
  const hassanHash = hashSimple('hassan123');
  ...
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u4', '100', 'omar', omarHash, 'employee'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u5', '200', 'admin', adminHash, 'admin'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u6', '200', 'ali', aliHash, 'employee'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u7', '300', 'admin', adminHash, 'admin'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u8', '300', 'nabil', nabilHash, 'employee'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u9', '300', 'fatima', fatimaHash, 'employee'] });
  await turso.execute({ sql: 'INSERT INTO users (id, tenant_id, username, password_hash, role) VALUES (?, ?, ?, ?, ?)', args: ['u10', '100', 'hassan', hassanHash, 'employee'] });
  ...
  for (const p of products100) {
    ...
    await turso.execute({ sql: 'INSERT INTO inventory_movements (id, tenant_id, product_id, type, quantity, created_at) VALUES (?, ?, ?, ?, ?, ?)', args: [`im_${p.id}`, '100', p.id, 'in', p.qty, daysAgo(now, 45)] });
  }
  ...
  for (const p of products200) {
    ...
    await turso.execute({ sql: 'INSERT INTO inventory_movements (id, tenant_id, product_id, type, quantity, created_at) VALUES (?, ?, ?, ?, ?, ?)', args: [`im_${p.id}`, '200', p.id, 'in', p.qty, daysAgo(now, 30)] });
  }
  ...
  for (const p of products300) {
    await turso.execute({ sql: 'INSERT INTO products (id, tenant_id, name, barcode, purchase_price, sale_price, currency, quantity, category, unit_of_measure, min_stock, max_stock, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', args: [p.id, '300', p.name, p.barcode, p.buy, p.sell, 'SAR', p.qty, p.cat, p.unit, p.min, p.max, p.expiry] });
    await turso.execute({ sql: 'INSERT INTO inventory_movements (id, tenant_id, product_id, type, quantity, created_at) VALUES (?, ?, ?, ?, ?, ?)', args: [`im_${p.id}`, '300', p.id, 'in', p.qty, daysAgo(now, 20)] });
  }
  ...
  for (const c of cust200) {
    await turso.execute({ sql: 'INSERT INTO customers (id, tenant_id, name, phone, email, address, loyalty_points, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', args: [c.id, '200', c.name, c.phone, c.email, c.address, c.points, daysAgo(now, c.created)] });
  }
  ...
  for (const c of cust300) {
    await turso.execute({ sql: 'INSERT INTO customers (id, tenant_id, name, phone, email, address, loyalty_points, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', args: [c.id, '300', c.name, c.phone, c.email, c.address, c.points, daysAgo(now, c.created)] });
  }
  ...
  for (const sale of salesData) {
    ...
    await turso.execute({ sql: 'INSERT INTO sales (id, tenant_id, total, discount, discount_type, final_total, created_by, created_at, customer_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', args: [sale.id, '100', total, discount, discount > 0 ? 'percentage' : null, finalTotal, sale.by, saleDate, sale.cust] });
  ...
  const sales200 = [
    { id: 's201', day: 1, by: 'ali', cust: 'c201', items: [{ pid: 'p201', qty: 2, price: 24 }, { pid: 'p205', qty: 3, price: 10 }] },
    { id: 's202', day: 2, by: 'admin', cust: null, items: [{ pid: 'p203', qty: 1, price: 48 }, { pid: 'p206', qty: 4, price: 7 }] },
    { id: 's203', day: 3, by: 'ali', cust: 'c202', items: [{ pid: 'p204', qty: 2, price: 32 }, { pid: 'p207', qty: 5, price: 4 }] },
    { id: 's204', day: 5, by: 'admin', cust: 'c203', items: [{ pid: 'p201', qty: 3, price: 24 }, { pid: 'p202', qty: 2, price: 19 }] },
    { id: 's205', day: 7, by: 'ali', cust: null, items: [{ pid: 'p208', qty: 4, price: 16 }, { pid: 'p205', qty: 3, price: 10 }] },
    { id: 's206', day: 8, by: 'admin', cust: 'c204', items: [{ pid: 'p203', qty: 2, price: 48 }, { pid: 'p204', qty: 1, price: 32 }] },
    { id: 's207', day: 10, by: 'ali', cust: 'c205', items: [{ pid: 'p206', qty: 6, price: 7 }, { pid: 'p207', qty: 4, price: 4 }] },
    { id: 's208', day: 12, by: 'admin', cust: null, items: [{ pid: 'p201', qty: 4, price: 24 }, { pid: 'p202', qty: 3, price: 19 }] },
    { id: 's209', day: 14, by: 'ali', cust: 'c201', items: [{ pid: 'p208', qty: 3, price: 16 }, { pid: 'p205', qty: 2, price: 10 }] },
    { id: 's210', day: 0, by: 'admin', cust: 'c203', items: [{ pid: 'p203', qty: 3, price: 48 }, { pid: 'p204', qty: 2, price: 32 }, { pid: 'p206', qty: 5, price: 7 }] },
  ];

  for (const sale of sales200) {
    const saleDate = daysAgo(now, sale.day);
    const total = sale.items.reduce((sum, i) => sum + i.qty * i.price, 0);
    await turso.execute({ sql: 'INSERT INTO sales (id, tenant_id, total, created_by, created_at, customer_id) VALUES (?, ?, ?, ?, ?, ?)', args: [sale.id, '200', total, sale.by, saleDate, sale.cust] });
    for (let idx = 0; idx < sale.items.length; idx++) {
      const item = sale.items[idx];
      await turso.execute({ sql: 'INSERT INTO sale_items (id, sale_id, product_id, qty, price) VALUES (?, ?, ?, ?, ?)', args: [`${sale.id}_item${idx}`, sale.id, item.pid, item.qty, item.price] });
      await turso.execute({ sql: 'INSERT INTO inventory_movements (id, tenant_id, product_id, type, quantity, created_at) VALUES (?, ?, ?, ?, ?, ?)', args: [`mov_${sale.id}_${idx}`, '200', item.pid, 'out', item.qty, saleDate] });
    }
  }

  const sales300 = [
    { id: 's301', day: 2, by: 'nabil', cust: 'c301', items: [{ pid: 'p301', qty: 1, price: 95 }, { pid: 'p303', qty: 2, price: 36 }] },
    { id: 's302', day: 5, by: 'admin', cust: null, items: [{ pid: 'p302', qty: 3, price: 26 }, { pid: 'p304', qty: 2, price: 34 }] },
    { id: 's303', day: 8, by: 'nabil', cust: 'c302', items: [{ pid: 'p301', qty: 2, price: 95 }, { pid: 'p302', qty: 1, price: 26 }] },
    { id: 's304', day: 11, by: 'admin', cust: 'c301', items: [{ pid: 'p303', qty: 3, price: 36 }, { pid: 'p304', qty: 3, price: 34 }] },
    { id: 's305', day: 0, by: 'nabil', cust: null, items: [{ pid: 'p301', qty: 1, price: 95 }, { pid: 'p302', qty: 2, price: 26 }, { pid: 'p304', qty: 2, price: 34 }] },
  ];

  for (const sale of sales300) {
    const saleDate = daysAgo(now, sale.day);
    const total = sale.items.reduce((sum, i) => sum + i.qty * i.price, 0);
    await turso.execute({ sql: 'INSERT INTO sales (id, tenant_id, total, created_by, created_at, customer_id) VALUES (?, ?, ?, ?, ?, ?)', args: [sale.id, '300', total, sale.by, saleDate, sale.cust] });
    for (let idx = 0; idx < sale.items.length; idx++) {
      const item = sale.items[idx];
      await turso.execute({ sql: 'INSERT INTO sale_items (id, sale_id, product_id, qty, price) VALUES (?, ?, ?, ?, ?)', args: [`${sale.id}_item${idx}`, sale.id, item.pid, item.qty, item.price] });
      await turso.execute({ sql: 'INSERT INTO inventory_movements (id, tenant_id, product_id, type, quantity, created_at) VALUES (?, ?, ?, ?, ?, ?)', args: [`mov_${sale.id}_${idx}`, '300', item.pid, 'out', item.qty, saleDate] });
    }
  }
  ...
  await turso.execute({ sql: 'INSERT INTO exchange_rates (id, tenant_id, sar_to_yer, updated_at) VALUES (?, ?, ?, ?)', args: ['rate2', '200', 396.0, daysAgo(now, 3)] });
  await turso.execute({ sql: 'INSERT INTO exchange_rates (id, tenant_id, sar_to_yer, updated_at) VALUES (?, ?, ?, ?)', args: ['rate3', '300', 395.5, daysAgo(now, 5)] });
  ...
  for (let i = 0; i < notifications.length; i++) {
    ...
    await turso.execute({ sql: 'INSERT INTO notifications (id, tenant_id, title, message, type, is_read, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', args: [`notif_${i + 1}`, n.tid, n.title, n.message, n.type, i < 3 ? 1 : 0, daysAgo(now, notifications.length - i)] });
  }
  ...
  for (let i = 0; i < costs.length; i++) {
    const c = costs[i];
    await turso.execute({ sql: 'INSERT INTO financial_costs (id, tenant_id, category, description, amount, currency, cost_date, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', args: [`cost_${i + 1}`, c.tid, c.cat, c.desc, c.amount, 'SAR', daysAgo(now, c.day).split('T')[0], 'admin', daysAgo(now, c.day)] });
  }

  const coupons = [
    { id: 'coupon1', code: 'MUKALLA10', type: 'percentage', value: 10, maxUses: 200, used: 45, minTotal: 50, active: true, created: 60 },
    { id: 'coupon2', code: 'SAVE50', type: 'fixed', value: 50, maxUses: 100, used: 18, minTotal: 200, active: true, created: 30 },
    { id: 'coupon3', code: 'VIP100', type: 'fixed', value: 100, maxUses: 50, used: 8, minTotal: 500, active: true, created: 90 },
    { id: 'coupon4', code: 'RAMADAN20', type: 'percentage', value: 20, maxUses: 300, used: 120, minTotal: 100, active: true, created: 120 },
    { id: 'coupon5', code: 'NEWYEAR15', type: 'percentage', value: 15, maxUses: 150, used: 65, minTotal: 75, active: true, created: 45 },
    { id: 'coupon6', code: 'LOYALTY25', type: 'percentage', value: 25, maxUses: 30, used: 12, minTotal: 300, active: true, created: 15 },
  ];
  ...
  for (const d of debts) {
    await turso.execute({ sql: 'INSERT INTO debts (id, tenant_id, customer_id, amount, currency, description, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', args: [d.id, '100', d.cust, d.amount, 'SAR', d.desc, d.status, d.by, daysAgo(now, d.day)] });
    if (d.status === 'partial') {
      await turso.execute({ sql: 'INSERT INTO debt_logs (id, tenant_id, debt_id, action, amount, performed_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', args: [`dl_${d.id}_1`, '100', d.id, 'partial_payment', d.amount * 0.4, 'admin', daysAgo(now, d.day - 3)] });
    }
    if (d.status === 'paid') {
      await turso.execute({ sql: 'INSERT INTO debt_logs (id, tenant_id, debt_id, action, amount, performed_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', args: [`dl_${d.id}_1`, '100', d.id, 'full_payment', d.amount, 'admin', daysAgo(now, d.day - 1)] });
    }
  }
  ...
  const promotions = [
    ...
    { id: 'promo3', name: 'عرض عيد الأضحى - خصم 10% على كل شيء', type: 'discount', pid: null, minQty: 0, freeQty: 0, discount: 10, from: -10, to: 10 },
  ...
  for (const pr of promotions) {
    await turso.execute({ sql: 'INSERT INTO promotions (id, tenant_id, name, type, product_id, min_qty, free_qty, discount_percent, valid_from, valid_to, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', args: [pr.id, '100', pr.name, pr.type, pr.pid, pr.minQty, pr.freeQty, pr.discount, daysAgo(now, -pr.from).split('T')[0], daysAgo(now, -pr.to).split('T')[0], 1] });
  }
  ...
  for (let i = 0; i < auditEntries.length; i++) {
    ...
    await turso.execute({ sql: 'INSERT INTO audit_logs (id, tenant_id, action, performed_by, created_at) VALUES (?, ?, ?, ?, ?)', args: [`log_${i + 1}`, tid, a.action, a.by, daysAgo(now, a.day)] });
  }
